...
echo "=== Find Transfer with IMEI Tracking ===\n";

// Tìm phiếu chuyển kho có transfer_code cụ thể
$transfer = StockTransfer::where('transfer_code', 'AUTO-689F58D195B9F')
    ->with(['items.productVariant', 'fromLocation', 'toLocation'])
    ->first();

if (!$transfer) {
    echo "Transfer AUTO-689F58D195B9F not found\n";
    exit;
}
...
foreach ($transfer->items as $item) {
    ...
    if ($item->productVariant->has_serial) {
        ...
        $remainingSerials = InventorySerial::where('product_variant_id', $item->product_variant_id)
            ->where('store_location_id', $transfer->from_store_location_id)
