...
@section('title', 'Gói hàng hôm nay')
...
<div x-data="shipperDashboard()">
    <header class="page-header p-5 bg-white flex justify-between items-center">
        <div>
            ...
            {{-- Biến $shipper đã được sửa ở bước trước --}}
            ...
            <button
                ...
                <template x-if="unreadCount > 0">
                    <span class="absolute -top-0.5 -right-0.5 flex h-4 w-4">
                        ...
                        <span class="relative inline-flex items-center justify-center text-xs text-white rounded-full h-4 w-4 bg-red-500" x-text="unreadCount"></span>
                    </span>
                </template>
            </button>
    ...
    <nav class="page-header sticky top-0 bg-white z-10">
        <div class="relative flex border-b border-gray-200">
            {{-- SỬA Ở ĐÂY: Dùng biến $fulfillmentsToPickup --}}
            <button data-tab="pickup" class="tab-btn flex-1 p-4 text-sm font-semibold text-indigo-600">Cần Lấy ({{ $fulfillmentsToPickup->count() }})</button>
            {{-- SỬA Ở ĐÂY: Dùng biến $fulfillmentsInTransit --}}
            <button data-tab="shipping" class="tab-btn flex-1 p-4 text-sm font-semibold text-gray-500">Đang Giao ({{ $fulfillmentsInTransit->count() }})</button>
    ...
    <main class="page-content p-4 space-y-3 bg-gray-50">
        ...
        <div id="pickup-list" class="tab-content space-y-3">
            {{-- SỬA Ở ĐÂY: Lặp qua $fulfillmentsToPickup và truyền $fulfillment->order vào partial --}}
            @forelse($fulfillmentsToPickup as $fulfillment)
                @include('shipper.partials.order_card', ['fulfillment' => $fulfillment])
            @empty
                <p class="text-center text-gray-500 pt-10">Không có gói hàng nào cần lấy.</p>
            @endforelse
        </div>

        <div id="shipping-list" class="tab-content hidden space-y-3">
            @forelse($fulfillmentsInTransit as $fulfillment)
                @include('shipper.partials.order_card', ['fulfillment' => $fulfillment])
            @empty
                <p class="text-center text-gray-500 pt-10">Không có gói hàng nào đang giao.</p>
            @endforelse
        </div>
    </main>
...
<script>
    // Phần script AlpineJS và các xử lý tab giữ nguyên
    function shipperDashboard() {
        return {
            isScannerOpen: false,
            // THAY ĐỔI 1: Đổi tên biến này cho đúng mục đích
            ...
            init() {
                ...
                // Không cần window.addEventListener ở đây khi dùng AlpineJS
            },
            ...
            // THAY ĐỔI 2: Sửa tên tham số và biến được gán
            openBarcodeScanner(fulfillmentId) {
                this.currentFulfillmentId = fulfillmentId; // Gán vào biến đã đổi tên
            ...
            closeBarcodeScanner() {
                ...
                // THAY ĐỔI 3: Reset đúng biến
                this.currentFulfillmentId = null; 
            },
            ...
            startScanning() {
                ...
                const onScanSuccess = (decodedText, decodedResult) => {
                    ...
                    // Gọi thẳng hàm handle, không cần dispatch event
            ...
            // THAY ĐỔI 4: Toàn bộ hàm này dùng tên biến mới
            handleBarcodeScanned(code) {
                // Kiểm tra đúng biến
                if (!this.currentFulfillmentId) return; 
                
                console.log('Scanning barcode:', code, 'for fulfillment:', this.currentFulfillmentId);
                
                // Gửi request với đúng biến
                fetch(`/shipper/fulfillments/${this.currentFulfillmentId}/update-status`, {
                    method: 'PATCH',
                    headers: {
                ...
                .catch(error => {
                    ...
                    this.showAlert(error.message || 'Lỗi kết nối. Vui lòng thử lại.', 'error');
                });
            },
