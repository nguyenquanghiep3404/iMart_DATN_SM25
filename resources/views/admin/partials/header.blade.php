@php
    ...
    // Giả lập số lượng thông báo chưa đọc
    $unreadNotificationsCount = 3;

    // Giả lập danh sách thông báo gần đây
    $recentNotifications = [
        [
            'id' => 1,
            'title' => 'Đơn hàng mới',
...
<header class="relative z-30 bg-white dark:bg-slate-800 dark:border-b dark:border-slate-700 shadow-sm print:hidden">
    <div class="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
        <div class="flex items-center space-x-4">
            ...
            <button type="button"
                ...
                {{-- Updated click handler to use the new function --}}
            ...
            <div class="hidden md:block">
                <form action="#">
                    <div class="relative group">
                        ...
                        <input
                            class="w-52 lg:w-72 h-10 pl-10 pr-4 bg-slate-100 dark:bg-slate-700 border border-transparent rounded-lg focus:bg-white dark:focus:bg-slate-600 focus:border-indigo-400 focus:ring-indigo-400 focus:ring-1 transition-all duration-300 outline-none text-slate-700 dark:text-slate-200"
        ...
        <div class="flex items-center space-x-2 sm:space-x-3">
            ...
            <div class="relative" x-data="{
                ...
                markAsRead() {
                    if (this.unreadCount > 0) {
                        // Uncomment and adjust the fetch call when backend is ready
                        /*
                        ...
                        */
                        // For demo purposes, just reset the count locally
                        this.unreadCount = 0;
                    }
                }
            }">
                <!-- Nút chuông -->
                <button @click.stop="notificationOpen = !notificationOpen; if (notificationOpen) markAsRead()"
                ...
                <div x-show="notificationOpen" @click.outside="notificationOpen = false"
                        ...
                        @forelse($recentNotifications ?? [] as $notification)
                            <a href="#" class="flex items-start p-3 hover:bg-gray-700/50 transition-colors">
                                ...
                                <div class="ml-3 flex-1">
                                    <p class="text-sm font-medium text-black-200">
                    ...
                    <div class="px-4 py-2 border-t border-slate-200 dark:border-slate-600 text-center">
                        <a href="#"
                            class="block w-full text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300">
                            Xem tất cả
                        </a>
                    </div>
                </div>
            ...
            <div class="relative w-[70%] flex justify-end items-center" x-data="{ userOption: false }">
                ...
                <div x-show="userOption" @click.outside="userOption = false"
                    ...
                    style="display: none;" id="userDropdown">
                    <!-- User Info -->
                    ...
                    <ul class="text-slate-700 text-base">
                        <li>
                            <a href="{{ route('profile.edit') }}"
                                class="px-5 py-2 w-full block hover:bg-gray-100 rounded-md hover:text-indigo-600">Cài
                                đặt tài khoản</a>
                        </li>
                        <li>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit"
                                    class="text-left px-5 py-2 w-full block hover:bg-red-100 rounded-md hover:text-red-600">
                                    Đăng xuất
                                </button>
                            </form>
                        </li>
    ...
    {{-- Removed the old toggle button and script --}}
</header>