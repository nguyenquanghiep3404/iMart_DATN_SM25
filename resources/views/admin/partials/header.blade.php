...
<header x-data="{ searchOpen: false }"
            ...
            <div class="relative" x-data="{ notificationOpen: false, unreadCount: {{ $unreadNotificationsCount }} }">
                ...
                <div x-show="notificationOpen" @click.outside="notificationOpen = false"
                            ...
                            <a href="#" class="flex items-start p-3 hover:bg-gray-700/50 transition-colors">
                                <div class="flex-shrink-0 w-10 h-10 bg-{{ $notification['color'] ?? 'gray' }}-500/20 text-{{ $notification['color'] ?? 'gray' }}-400 rounded-full flex items-center justify-center">
                                    ...
                                    @elseif (isset($notification['icon']) && $notification['icon'] === 'warning')
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="m21.73 18-8-14a2 2 0 0 0-3.46 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"></path><line x1="12" x2="12" y1="9" y2="13"></line><line x1="12" x2="12.01" y1="17" y2="17"></line></svg>
                                    @else
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><line x1="12" x2="12" y1="2" y2="22" /><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" /></svg>
                                    @endif
                                </div>
                                <div class="ml-3 flex-1">
                                    <p class="text-sm font-medium text-slate-800 dark:text-slate-100">{{ $notification['title'] ?? 'Không có tiêu đề' }}</p>
                                    <p class="text-sm text-slate-600 dark:text-slate-300">{{ $notification['message'] ?? 'Không có nội dung' }}</p>
                    ...
                    <div class="px-4 py-2 border-t border-slate-200 dark:border-slate-600 text-center">
                        <a href="#" class="block w-full text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300">Xem tất cả</a>
                    </div>
                </div>
            </div>
            ...
            <div class="relative flex items-center" x-data="{ userOption: false }">
                ...
                <div x-show="userOption" @click.outside="userOption = false"
                    ...
                    <div class="flex items-center space-x-3 border-b border-gray-200 dark:border-slate-600 pb-3 mb-2">
                        ...
                        <div>
                            <h5 class="text-base mb-1 leading-none text-slate-800 dark:text-slate-100">{{ $user->name }}</h5>
    ...
    <div x-show="searchOpen" x-trap.noscroll="searchOpen" style="display: none;"
        ...
        <div @click.away="searchOpen = false"
            class="relative w-full max-w-lg mt-12 transform"
            x-transition:enter="transition ease-out duration-200"
            ...
            <form action="#">
                <div class="relative">
                    ...
                    <button type="button" @click="searchOpen = false" aria-label="Đóng tìm kiếm" class="absolute right-5 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-800">
