...
<div id="add-staff-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-3xl mx-4 max-h-[90vh] overflow-y-auto" id="modal-content">
        ...
        <div class="flex justify-between items-center p-6 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">Thêm Nhân Viên Mới</h3>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    employeeSearchInput.addEventListener('input', function() {
        ...
        const noResultsRow = staffTableBody.querySelector('tr[colspan]');
    ...
    if (addStaffForm) {
        ...
        addStaffForm.addEventListener('submit', function(e) {
            ...
            const errorFields = ['name','email','phone','status','store_location_id','province','district'];
            ...
            // console.log('DEBUG: Form field elements:', {
            //     nameField,
            //     emailField,
            //     phoneField,
            //     statusField
            ...
            // console.log('DEBUG: Dropdown field elements:', {
            //     provinceInput,
            //     provinceSelect,
            //     districtSelect,
            //     storeSelect
            // });
            // console.log('DEBUG: Dropdown field values:', {
            //     provinceInputValue: provinceInput?.value,
            //     provinceSelectValue: provinceSelect?.value,
            //     districtSelectValue: districtSelect?.value,
            //     storeSelectValue: storeSelect?.value
            // });
            ...
            // console.log('DEBUG: Dropdown validation values:', {
            //     province: province,
            //     district: district,
            //     storeLocationId: storeLocationId,
            //     hasProvinceInput: !!provinceInput,
            //     hasProvince: !!province,
            //     hasDistrict: !!district,
            //     hasStoreLocationId: !!storeLocationId,
            //     provinceLength: province.length,
            //     districtLength: district.length,
            //     storeLocationIdLength: storeLocationId.length
            // });
            
            if (!name) {
                // console.log('DEBUG: Name validation failed');
                const errorNameEl = document.getElementById('error-name');
                // console.log('DEBUG: error-name element:', errorNameEl);
                if (errorNameEl) {
            ...
            if (!email) {
                ...
                // console.log('DEBUG: error-email element:', errorEmailEl);
                if (errorEmailEl) {
            ...
            if (!phone) {
                ...
                // console.log('DEBUG: error-phone element:', errorPhoneEl);
                if (errorPhoneEl) {
            ...
            if (!status) {
                // console.log('DEBUG: Status validation failed - empty');
                const errorStatusEl = document.getElementById('error-status');
                // console.log('DEBUG: error-status element:', errorStatusEl);
                if (errorStatusEl) {
                    errorStatusEl.innerHTML = '<div class="text-red-600 text-sm">Vui lòng chọn trạng thái</div>';
                }
                hasError = true;
            }
            ...
            if (!provinceInput) {
                if (!province) {
                    ...
                    // console.log('DEBUG: error-province element:', errorProvinceEl);
                    if (errorProvinceEl) {
                ...
                if (!district) {
                    // console.log('DEBUG: District validation failed');
                    const errorDistrictEl = document.getElementById('error-district');
                    // console.log('DEBUG: error-district element:', errorDistrictEl);
                    if (errorDistrictEl) {
                ...
                if (!storeLocationId) {
                    // console.log('DEBUG: Store location validation failed');
                    const errorStoreEl = document.getElementById('error-store_location_id');
                    // console.log('DEBUG: error-store_location_id element:', errorStoreEl);
                    if (errorStoreEl) {
            ...
            .then(async response => {
                ...
                if (!response.ok) {
                    ...
                    if (data && data.errors) {
                        ...
                        // console.log('DEBUG: Validation errors type:', typeof data.errors);
                        // console.log('DEBUG: Validation errors keys:', Object.keys(data.errors));
                        Object.entries(data.errors).forEach(([field, arr]) => {
                            const el = document.getElementById('error-' + field);
                            // console.log('DEBUG: Setting error for field:', field, 'element:', el, 'message:', arr[0]);
