@if (!isset($store)) @php $store = null; @endphp @endif
    ...
    <div class="bg-white rounded-lg shadow-2xl p-8 w-full max-w-2xl m-4" id="modal-content">
        ...
        <form id="add-staff-form">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label class="block text-gray-700 text-sm font-semibold mb-2" for="new-staff-status">Trạng thái</label>
                    <select id="new-staff-status" name="status" class="w-full px-4 py-2 border rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    modalDistrictSelect.addEventListener('change', function() {
        ...
        if (provinceCode && districtCode) {
            fetch(`/api/store-locations/stores?province_code=${provinceCode}&district_code=${districtCode}`)
                .then(response => response.json())
                .then(data => {
                    modalStoreSelect.innerHTML = '<option value="">-- Chọn cửa hàng --</option>';
    ...
    addStaffForm.addEventListener('submit', function(e) {
        ...
        .catch(() => {
            document.getElementById('staff-form-errors').innerHTML = '<div class="text-red-600 text-sm mb-1">Lỗi kết nối server hoặc lỗi JS.</div>';
        });
    });
