        ...
        <div class="bg-white p-8 rounded-xl shadow-sm">
            <form action="{{ route('admin.shippers.store') }}" method="POST">
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}"
                                class="w-full py-2 px-3 border border-gray-300 rounded-lg">
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="province" class="block text-sm font-medium text-gray-700 mb-1">Tỉnh/Thành phố <span
                                    class="text-red-500">*</span></label>
                            <select name="province_code" id="province"
                                class="w-full py-2 px-3 border border-gray-300 bg-white rounded-lg"
                                @if (isset($selectedProvince)) disabled @endif>
                                <option value="">Chọn Tỉnh/Thành phố</option>
                                @foreach ($provinces ?? [] as $province)
                                    <option value="{{ $province->code }}" @selected(old('province_code', $selectedProvince->code ?? '') == $province->code)>
                        ...
                        <div>
                            <label for="warehouse" class="block text-sm font-medium text-gray-700 mb-1">Kho làm việc <span
                                    class="text-red-500">*</span></label>
                ...
                <div class="pt-8 flex justify-end space-x-3">
                    @if (request()->has('warehouse_id'))
                        <a href="{{ route('admin.shippers.warehouse.show', request('warehouse_id')) }}"
                            class="px-5 py-2 bg-gray-200 text-gray-700 rounded-lg font-semibold">Hủy</a>
                    @else
                        <a href="{{ route('admin.shippers.index') }}"
                            class="px-5 py-2 bg-gray-200 text-gray-700 rounded-lg font-semibold">Hủy</a>
                    @endif
                    <button type="submit" class="px-5 py-2 bg-indigo-600 text-white rounded-lg font-semibold">Tạo nhân
                        viên</button>
                </div>
            </form>
        </div>
