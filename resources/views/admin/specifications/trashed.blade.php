...
<div class="body-content px-4 sm:px-6 md:px-8 py-8">
    <div class="container mx-auto max-w-full">
        ...
        {{-- PAGE HEADER --}}
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-800">Thùng rác - Thông số kỹ thuật</h1>
        ...
        <div class="card-custom">
            <div class="card-custom-header">
                <div class="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center w-full">
                    <h3 class="card-custom-title">Danh sách thông số đã xóa ({{ $specifications->total() }})</h3>
                    <a href="{{ route('admin.specifications.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left mr-2"></i>Quay lại</a>
                </div>
            </div>
                    ...
                    <table class="table-custom table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ optional($specification->group)->name ?? 'Không có nhóm' }}</td>
                                <td>{{ $specification->deleted_at->format('d/m/Y H:i') }}</td>
                                <td class="text-center">
                                    <div class="inline-flex space-x-1">
                                        <button type="button" class="btn btn-success btn-sm" title="Khôi phục" onclick="openModal('restoreModal{{ $specification->id }}')"><i class="fas fa-undo"></i></button>
                                        <button type="button" class="btn btn-danger btn-sm" title="Xóa vĩnh viễn" onclick="openModal('forceDeleteModal{{ $specification->id }}')"><i class="fas fa-trash-alt"></i></button>
                                    </div>
                                </td>
                            </tr>
                            
                            {{-- RESTORE CONFIRMATION MODAL --}}
                            <div id="restoreModal{{ $specification->id }}" class="modal" tabindex="-1">
                                <div class="modal-content animated-modal">
                                    {{-- FIX: Using url() helper as a workaround for route naming issues. --}}
                                    <form action="{{ url('admin/specifications/' . $specification->id . '/restore') }}" method="POST">
                                        @csrf
                                        <div class="modal-body text-center p-6">
                                            <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-green-100 mb-4">
                                                <i class="fas fa-undo fa-2x text-green-500"></i>
                                            </div>
                                            <h5 class="text-xl font-semibold text-gray-800">Khôi phục thông số?</h5>
                                            <p class="text-gray-600 mt-2">Bạn có chắc chắn muốn khôi phục thông số<br>"<strong>{{ $specification->name }}</strong>"?</p>
                                        </div>
                                        <div class="modal-footer justify-center">
                                            <button type="button" class="btn btn-secondary" onclick="closeModal('restoreModal{{ $specification->id }}')">Hủy bỏ</button>
                            ...
                            <div id="forceDeleteModal{{ $specification->id }}" class="modal" tabindex="-1">
                                <div class="modal-content animated-modal">
                                    {{-- FIX: Using url() helper as a workaround for route naming issues. --}}
                                    <form action="{{ url('admin/specifications/' . $specification->id . '/force-delete') }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <div class="modal-body text-center p-6">
                                            ...
                                            <p class="text-gray-600 mt-2">Bạn có chắc chắn muốn xóa vĩnh viễn thông số<br>"<strong>{{ $specification->name }}</strong>"?</p>
                                            <p class="text-red-600 mt-2 text-sm font-bold">Hành động này không thể hoàn tác!</p>
                                        </div>
                                        <div class="modal-footer justify-center">
                                            <button type="button" class="btn btn-secondary" onclick="closeModal('forceDeleteModal{{ $specification->id }}')">Hủy bỏ</button>
