...
@section('title', 'Quản lý Thông số kỹ thuật')

@push('styles')
    {{-- Bạn có thể giữ nguyên các style đã có trong file spec_group_index_view --}}
    <style>
...
<div class="body-content px-4 sm:px-6 md:px-8 py-8">
    <div class="container mx-auto max-w-full">
        ...
        {{-- PAGE HEADER --}}
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-800">Quản lý Thông số kỹ thuật</h1>
            <nav aria-label="breadcrumb" class="mt-2">
                <ol class="flex text-sm text-gray-500">
                    ...
                    <li class="breadcrumb-item active text-gray-700 font-medium" aria-current="page">Thông số kỹ thuật</li>
                </ol>
            </nav>
        </div>
        ...
        <div class="card-custom">
            <div class="card-custom-header">
                <div class="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center w-full">
                    {{-- Sửa lại biến $groups thành $specifications --}}
                    <h3 class="card-custom-title">Danh sách Thông số ({{ $specifications->total() }})</h3>
                    <div class="flex items-center space-x-2">
                        <a href="{{ route('admin.specifications.trashed') }}" class="btn btn-warning btn-sm" title="Thùng rác"><i class="fas fa-trash-alt mr-1"></i>Thùng rác</a>
                        <a href="{{ route('admin.specifications.create') }}" class="btn btn-primary"><i class="fas fa-plus mr-2"></i>Thêm Thông số</a>
                    </div>
                </div>
            </div>
            <div class="card-custom-body">
                {{-- FILTERS --}}
                <form action="{{ route('admin.specifications.index') }}" method="GET" class="mb-6">
                    {{-- Thêm các bộ lọc nếu cần, ví dụ lọc theo nhóm --}}
                </form>
                ...
                <div class="overflow-x-auto border border-gray-200 rounded-lg">
                    <table class="table-custom table-striped">
                        ...
                        <tbody>
                            {{-- Sửa vòng lặp từ $groups as $group thành $specifications as $specification --}}
                            @forelse ($specifications as $specification)
                            <tr>
                                <td>{{ ($specifications->currentPage() - 1) * $specifications->perPage() + $loop->iteration }}</td>
                                <td>
                                    <a href="{{ route('admin.specifications.edit', $specification) }}" class="font-semibold text-indigo-600 hover:text-indigo-800">{{ $specification->name }}</a>
                                </td>
                                <td>{{ $specification->group->name ?? 'N/A' }}</td>
                                <td class="text-center">{{ $specification->type }}</td>
                                <td class="text-center">{{ $specification->order }}</td>
                                <td>{{ $specification->created_at->format('d/m/Y H:i') }}</td>
                                <td class="text-center">
                                    <div class="inline-flex space-x-1">
                                        <a href="{{ route('admin.specifications.edit', $specification) }}" class="btn btn-primary btn-sm" title="Chỉnh sửa"><i class="fas fa-edit"></i></a>
                                        <button type="button" class="btn btn-danger btn-sm" title="Xóa" onclick="openModal('deleteModal{{ $specification->id }}')"><i class="fas fa-trash"></i></button>
                                    </div>
                                </td>
                            </tr>
                            
                            {{-- DELETE CONFIRMATION MODAL --}}
                            <div id="deleteModal{{ $specification->id }}" class="modal" tabindex="-1">
                                <div class="modal-content animated-modal">
                                    <form action="{{ route('admin.specifications.destroy', $specification->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <div class="modal-body text-center p-6">
                                            ...
                                            <p class="text-gray-600 mt-2">Bạn có chắc chắn muốn chuyển thông số<br>"<strong>{{ $specification->name }}</strong>" vào thùng rác không?</p>
                                        </div>
                                        <div class="modal-footer justify-center">
                                            <button type="button" class="btn btn-secondary" onclick="closeModal('deleteModal{{ $specification->id }}')">Hủy bỏ</button>
                            ...
                            <tr>
                                <td colspan="7" class="text-center py-10 text-gray-500">
                                    <div class="flex flex-col items-center">
                                        <i class="fas fa-microchip fa-3x mb-3 text-gray-400"></i>
                                        <p class="text-lg font-medium">Không tìm thấy thông số nào.</p>
                                        <p class="text-sm">Hãy <a href="{{ route('admin.specifications.create') }}" class="text-indigo-600 hover:underline">thêm thông số mới</a> để bắt đầu.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
            ...
            <div class="card-custom-footer">
                <div class="flex flex-col gap-4 md:flex-row md:justify-between md:items-center w-full">
                    <p class="text-sm text-gray-700 leading-5">
                        Hiển thị từ <span class="font-medium">{{ $specifications->firstItem() }}</span> đến <span class="font-medium">{{ $specifications->lastItem() }}</span> trên tổng số <span class="font-medium">{{ $specifications->total() }}</span> kết quả
                    </p>
                    <div>
                        {!! $specifications->appends(request()->query())->links() !!}
                    </div>
                </div>
            </div>
            @endif
...
@push('scripts')
    {{-- Giữ nguyên script modal và toast --}}
    <script>
