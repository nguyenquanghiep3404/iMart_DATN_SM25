...
@section('title', 'Chỉnh sửa Thông số kỹ thuật')
...
<div class="body-content px-4 sm:px-6 md:px-8 py-8">
    <div class="container mx-auto max-w-4xl">
        ...
        {{-- PAGE HEADER --}}
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-800">Chỉnh sửa Thông số kỹ thuật</h1>
        ...
        <form action="{{ route('admin.specifications.update', $specification) }}" method="POST">
            ...
            <div class="card-custom">
                <div class="card-custom-header">
                    <h3 class="card-custom-title">Thông tin chi tiết</h3>
                </div>
                <div class="card-custom-body">
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="specification_group_id" name="specification_group_id" class="form-select @error('specification_group_id') is-invalid @enderror" required>
                                ...
                                @foreach($groups as $group)
                                    <option value="{{ $group->id }}" {{ old('specification_group_id', $specification->specification_group_id) == $group->id ? 'selected' : '' }}>
                                        {{ $group->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('specification_group_id')
                        ...
                        <div>
                            ...
                            <input type="text" id="name" name="name" class="form-input @error('name') is-invalid @enderror" value="{{ old('name', $specification->name) }}" placeholder="Ví dụ: CPU, RAM, Camera chính" required>
                            @error('name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        {{-- Kiểu hiển thị --}}
                        <div>
                            <label for="type" class="form-label">Kiểu hiển thị <span class="text-red-500">*</span></label>
                            <select id="type" name="type" class="form-select @error('type') is-invalid @enderror" required>
                                <option value="text" {{ old('type', $specification->type) == 'text' ? 'selected' : '' }}>Văn bản (Text)</option>
                                <option value="textarea" {{ old('type', $specification->type) == 'textarea' ? 'selected' : '' }}>Đoạn văn (Textarea)</option>
                                <option value="boolean" {{ old('type', $specification->type) == 'boolean' ? 'selected' : '' }}>Có/Không (Boolean)</option>
                                <option value="select" {{ old('type', $specification->type) == 'select' ? 'selected' : '' }}>Lựa chọn (Select)</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3" class="form-input @error('description') is-invalid @enderror" placeholder="Giải thích thêm về thông số này (nếu cần)">{{ old('description', $specification->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" id="order" name="order" class="form-input @error('order') is-invalid @enderror" value="{{ old('order', $specification->order ?? 0) }}" style="width: 150px;">
                ...
                <div class="card-custom-footer">
                    <div class="flex justify-end space-x-3">
                        <a href="{{ route('admin.specifications.index') }}" class="btn btn-secondary">Hủy bỏ</a>
