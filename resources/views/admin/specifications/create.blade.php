...
@section('title', 'Thêm mới Thông số kỹ thuật')
...
<div class="body-content px-4 sm:px-6 md:px-8 py-8">
    <div class="container mx-auto max-w-4xl">
        ...
        {{-- PAGE HEADER --}}
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-800">Thêm mới Thông số kỹ thuật</h1>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="type" name="type" class="form-select @error('type') is-invalid @enderror" required>
                                <option value="text" {{ old('type', 'text') == 'text' ? 'selected' : '' }}>Văn bản (Text)</option>
                                <option value="textarea" {{ old('type') == 'textarea' ? 'selected' : '' }}>Đoạn văn (Textarea)</option>
                                <option value="boolean" {{ old('type') == 'boolean' ? 'selected' : '' }}>Có/Không (Boolean)</option>
                                <option value="select" {{ old('type') == 'select' ? 'selected' : '' }}>Lựa chọn (Select)</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3" class="form-input @error('description') is-invalid @enderror" placeholder="Giải thích thêm về thông số này (nếu cần)">{{ old('description') }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
