...
<style>
    ...
    .image-preview { /* Dùng chung cho ảnh bìa, thư viện */
        position: relative;
        width: 120px;
        height: 120px;
    ...
    .variant-image-preview-item { /* Class riêng cho từng item ảnh của biến thể */
        position: relative;
        width: 90px; /* Kích thước nhỏ hơn */
        height: 90px;
        border-radius: 0.5rem;
        overflow: hidden;
        border: 2px solid #e2e8f0; /* Viền mặc định */
        background-color: #f8fafc;
        display: flex;
        align-items: center;
        justify-content: center;
    ...
    .variant-image-preview-item.is-primary {
        border: 3px solid #2563eb; /* Viền xanh làm nổi bật ảnh chính */
        box-shadow: 0 0 8px rgba(37, 99, 235, 0.5);
    }
    .variant-image-preview-item .set-primary-btn {
        position: absolute;
        bottom: 4px; /* Vị trí nút đặt làm chính */
        left: 4px;
     ...
     .variant-image-preview-item .set-primary-btn:hover {
        background-color: rgba(37, 99, 235, 0.9); /* Màu khi hover */
     }
     .variant-image-preview-item .set-primary-btn i { /* Icon trong nút */
         width: 12px;
         height: 12px;
         margin-right: 3px;
     }
    ...
    .rich-text-placeholder { min-height: 250px; }
    .variant-card {
        border: 1px solid #e2e8f0;
        border-radius: 0.625rem;
        padding: 1.25rem;
    ...
    .form-check-input {
        height: 1.125rem; width: 1.125rem; margin-top: 0.125rem; border-color: #94a3b8;
    }
    ...
    .label-with-action {
        display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.625rem;
    }
                ...
                @endif
                 @foreach ($errors->get('variants.*') as $variantErrorKey => $messages)
                     @foreach ($messages as $message)
                         <p class="text-red-500 text-xs mt-1">Lỗi tại {{ $variantErrorKey }}: {{ $message }}</p>
                     @endforeach
                 @endforeach
    ...
    <div class="lg:col-span-1 space-y-6">
        ...
        <div class="card">
            ...
            <div class="input-group">
                ...
                @foreach ($errors->get('gallery_image_files.*') as $key => $messages) {{-- Loop through errors for each gallery image --}}
                     @foreach ($messages as $message)
...
<div id="messageModalIconContainer" class="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-blue-100">
...
if (isset($attributes) && $attributes instanceof \Illuminate\Support\Collection) {
    $jsAttributesData = $attributes->map(function($attr) {
        ...
        if (isset($attr->name)) { // Determine icon based on attribute name
...
function showMessageModal(title, text, type = 'info') { /* ... Keep existing ... */
...
if(messageModalCloseButton) { messageModalCloseButton.addEventListener('click', () => messageModal.classList.add('hidden')); }
function toggleButtonLoading(button, isLoading) { /* ... Keep existing ... */
if (!button) return;
...
async function callGeminiAPI(prompt, isStructured = false, schema = null) { /* ... Keep existing ... */
...
function getProductContext() { /* ... Keep existing ... */
...
if (firstVariantCard) {
const attributeSelects = firstVariantCard.querySelectorAll('select[name$="[attributes]"]'); // Simplified selector
let tempAttrs = [];
attributeSelects.forEach(select => {      const attrNameLabel = select.closest('.input-group')?.querySelector('label');
if (attrNameLabel) {
...
const seoSchema = { type: "OBJECT", properties: { meta_title: { type: "STRING" }, meta_description: { type: "STRING" }, meta_keywords: { type: "STRING" }}, required: ["meta_title", "meta_description", "meta_keywords"]};
if (generateShortDescBtn) { generateShortDescBtn.addEventListener('click', async () => { const context = getProductContext(); if (!context || !shortDescriptionTextarea) return; toggleButtonLoading(generateShortDescBtn, true); const prompt = `Viết một mô tả ngắn (khoảng 1-2 câu, tối đa 50 từ) thật hấp dẫn cho sản phẩm Apple sau: ${context}. Tập trung vào điểm nổi bật nhất, giọng văn chuyên nghiệp.`; const generatedText = await callGeminiAPI(prompt); if (generatedText) { shortDescriptionTextarea.value = generatedText; showMessageModal("Hoàn tất", "Đã tạo mô tả ngắn bằng AI!", "success"); } toggleButtonLoading(generateShortDescBtn, false); }); }
if (generateLongDescBtn) { generateLongDescBtn.addEventListener('click', async () => { const context = getProductContext(); if (!context || !longDescriptionTextarea || !shortDescriptionTextarea) return; const shortDesc = shortDescriptionTextarea.value.trim(); toggleButtonLoading(generateLongDescBtn, true); let prompt = `Viết một mô tả chi tiết (khoảng 3-5 đoạn văn, tập trung vào các tính năng, lợi ích chính và trải nghiệm người dùng) cho sản phẩm Apple sau: ${context}.`; prompt += shortDesc ? `\nCó thể dựa trên mô tả ngắn sau: "${shortDesc}".` : ''; prompt += `\nHãy viết bằng giọng văn chuyên nghiệp, phù hợp để đăng bán trên website thương mại điện tử. Tránh các từ ngữ quá quảng cáo, tập trung vào thông tin hữu ích.`; const generatedText = await callGeminiAPI(prompt); if (generatedText) { longDescriptionTextarea.value = generatedText; showMessageModal("Hoàn tất", "Đã tạo mô tả chi tiết bằng AI!", "success"); } toggleButtonLoading(generateLongDescBtn, false); }); }
if (generateAllSeoBtn) { generateAllSeoBtn.addEventListener('click', async () => { const context = getProductContext(); if (!context || !longDescriptionTextarea || !shortDescriptionTextarea || !metaTitleInput || !metaDescriptionTextarea || !metaKeywordsInput) return; const productDescription = longDescriptionTextarea.value.trim() || shortDescriptionTextarea.value.trim(); if (!productDescription) { showMessageModal("Thiếu thông tin", "Vui lòng có mô tả sản phẩm trước khi tạo SEO.", "error"); return; } toggleButtonLoading(generateAllSeoBtn, true); const prompt = `Dựa trên thông tin sản phẩm Apple: "${context}" và mô tả: "${productDescription}", hãy tạo các thẻ meta SEO (meta title, meta description, meta keywords) cho một trang web bán sản phẩm này.`; const seoData = await callGeminiAPI(prompt, true, seoSchema); if (seoData) { metaTitleInput.value = seoData.meta_title || ''; metaDescriptionTextarea.value = seoData.meta_description || ''; metaKeywordsInput.value = seoData.meta_keywords || ''; showMessageModal("Hoàn tất", "Đã tạo thông tin SEO bằng AI!", "success"); } toggleButtonLoading(generateAllSeoBtn, false); }); }
if (generateTagsBtn) { generateTagsBtn.addEventListener('click', async () => { const context = getProductContext(); if (!context || !tagsInput) return; toggleButtonLoading(generateTagsBtn, true); const prompt = `Gợi ý 5-7 thẻ (tags) phù hợp nhất cho sản phẩm Apple sau: ${context}. Các thẻ nên ngắn gọn, tập trung vào tên sản phẩm, dòng sản phẩm, tính năng chính hoặc đối tượng người dùng. Trả về dưới dạng danh sách các từ khóa cách nhau bởi dấu phẩy.`; const generatedTags = await callGeminiAPI(prompt); if (generatedTags) { tagsInput.value = generatedTags; showMessageModal("Hoàn tất", "Đã gợi ý thẻ sản phẩm bằng AI!", "success"); } toggleButtonLoading(generateTagsBtn, false); }); }
...
checkbox.onchange = updateSelectedAttributesForVariants;
    ...
    if (Array.isArray(oldVariantsData) && oldVariantsData.length > 0) {
        const usedInOld = oldVariantsData.some(oldVar => oldVar.attributes && typeof oldVar.attributes === 'object' && oldVar.attributes[attr.id.toString()]);
...
const productTypeRadio = document.querySelector('input[name="type"]:checked');
if (!productTypeRadio || (productTypeRadio.value === 'variable' && selectedProductAttributes.length === 0) ) {
...
variantCard.dataset.variantIndex = currentVariantIndex; // Store index
    ...
    let variantHeaderHTML = `<div class="variant-header"><div class="flex items-center"><i data-lucide="puzzle" class="mr-2 h-5 w-5 text-blue-600"></i><h4 class="variant-title">Biến Thể #${currentVariantIndex + 1}</h4></div><button type="button" class="remove-variant-btn btn btn-danger btn-sm p-1.5 rounded-md"><i data-lucide="trash-2" class="h-4 w-4"></i></button></div>`;
    ...
    selectedProductAttributes.forEach(attr => {
        ...
        attributesHTML += `<div class="input-group"><label for="variants_${currentVariantIndex}_attr_${attr.id}" class="text-sm font-medium">${attr.name} <span class="required-star">*</span></label><div class="input-with-icon"><i data-lucide="${attr.icon || 'tag'}" class="icon-prefix"></i><select name="variants[${currentVariantIndex}][attributes][${attr.id}]" id="variants_${currentVariantIndex}_attr_${attr.id}" class="select-field text-sm" required><option value="">Chọn ${attr.name}</option>${attr.attributeValues.map(val => `<option value="${val.id}">${val.value}</option>`).join('')}</select></div></div>`;
    });
    attributesHTML += '</div>';
    
    let variantFieldsHTML = `
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            <div class="input-group"><label for="variants_${currentVariantIndex}_sku" class="text-sm font-medium">SKU Biến Thể <span class="required-star">*</span></label><div class="input-with-icon"><i data-lucide="scan-barcode" class="icon-prefix"></i><input type="text" name="variants[${currentVariantIndex}][sku]" id="variants_${currentVariantIndex}_sku" class="input-field text-sm" required></div></div>
            <div class="input-group"><label for="variants_${currentVariantIndex}_price" class="text-sm font-medium">Giá Biến Thể <span class="required-star">*</span> (VNĐ)</label><div class="input-with-icon"><i data-lucide="dollar-sign" class="icon-prefix"></i><input type="number" name="variants[${currentVariantIndex}][price]" id="variants_${currentVariantIndex}_price" class="input-field text-sm" step="1000" min="0" required></div></div>
    ...
    if(removeBtn) removeBtn.addEventListener('click', function() { this.closest('.variant-card').remove(); updateDefaultVariantRadioAndHiddenFields(); });
...
function handleDefaultVariantChange(event) { /* ... Keep existing ... */
...
function updateDefaultVariantRadioAndHiddenFields() { /* ... Keep existing ... */
const defaultRadios = document.querySelectorAll('.variant-default-radio');
let oneIsChecked = false; defaultRadios.forEach(radio => { if (radio.checked) oneIsChecked = true; });
...
// --- Image Preview Functions (Cover, Gallery - Keep existing) ---
// function previewCoverImage(event) { /* ... Keep existing ... */ }
// function removeCoverImage(event) { /* ... Keep existing ... */ }
// function previewGalleryImages(event) { /* ... Keep existing ... */ }
// function removeSpecificGalleryImage(element, fileNameToRemove) { /* ... Keep existing ... */ }
// (Copied from previous response for completeness)
function previewCoverImage(event) { const container = document.getElementById('coverImagePreviewContainer'); if (!container) return; container.innerHTML = ''; const file = event.target.files[0]; if (file) { const reader = new FileReader(); reader.onload = function(e) { const div = document.createElement('div'); div.className = 'image-preview'; div.innerHTML = `<img src="${e.target.result}" alt="Cover Image Preview"><span class="remove-img-btn" onclick="removeCoverImage(event)"><i data-lucide="x" class="h-3 w-3"></i></span>`; container.appendChild(div); if (typeof lucide !== 'undefined') lucide.createIcons(); }; reader.readAsDataURL(file); } }
function removeCoverImage(event) { const coverImageInput = document.getElementById('cover_image_file'); if(coverImageInput) coverImageInput.value = ''; const previewElement = event.target.closest('.image-preview'); if(previewElement) previewElement.remove(); }
function previewGalleryImages(event) { const container = document.getElementById('galleryImagesPreviewContainer'); if (!container) return; container.innerHTML = ''; const files = event.target.files; Array.from(files).forEach((file, index) => { const reader = new FileReader(); reader.onload = function(e) { const div = document.createElement('div'); div.className = 'image-preview'; div.innerHTML = `<img src="${e.target.result}" alt="Gallery Image ${index + 1}"><span class="remove-img-btn" onclick="removeSpecificGalleryImage(this, '${file.name}')"><i data-lucide="x" class="h-3 w-3"></i></span>`; container.appendChild(div); }; reader.readAsDataURL(file); }); if (typeof lucide !== 'undefined') lucide.createIcons(); }
function removeSpecificGalleryImage(element, fileNameToRemove) { const galleryInput = document.getElementById('gallery_image_files'); if (!galleryInput) return; const currentFiles = Array.from(galleryInput.files); const newFiles = currentFiles.filter(file => file.name !== fileNameToRemove); const dataTransfer = new DataTransfer(); newFiles.forEach(file => dataTransfer.items.add(file)); galleryInput.files = dataTransfer.files; const previewElement = element.closest('.image-preview'); if (previewElement) previewElement.remove(); }
// --- NEW/UPDATED: Variant Image Handling ---
...
let currentPrimaryFilename = primaryImageInput.value; 
let newPrimaryCandidateFilename = null; 
...
files.forEach((file, idx) => {
    ...
    reader.onload = function(e) {
        ...
        div.innerHTML = `
            ...
            <button type="button" class="set-primary-btn" title="Đặt làm ảnh chính" onclick="setVariantPrimaryImage(${vIndex}, '${file.name}', this.closest('.variant-image-preview-item'))">
               <i data-lucide="star"></i> Đặt chính
            </button>
        `;
        previewContainer.appendChild(div);

        if (file.name === currentPrimaryFilename) {
            div.classList.add('is-primary');
            newPrimaryCandidateFilename = file.name; 
        }
        if (idx === 0 && !newPrimaryCandidateFilename && !currentPrimaryFilename) { 
             newPrimaryCandidateFilename = file.name;
        }
        
        if (idx === files.length -1) { // After last image is processed
            if (newPrimaryCandidateFilename) { 
                setVariantPrimaryImage(vIndex, newPrimaryCandidateFilename, previewContainer.querySelector(`.variant-image-preview-item[data-filename="${newPrimaryCandidateFilename}"]`));
            } else if (files.length > 0 && !primaryImageInput.value) { // If no primary was set from currentPrimaryFilename or as new, set first as primary
                 setVariantPrimaryImage(vIndex, files[0].name, previewContainer.querySelector(`.variant-image-preview-item[data-filename="${files[0].name}"]`));
            }
        }
        if (typeof lucide !== 'undefined') lucide.createIcons();
    }
...
// Fallback to set first image as primary if none is set after a small delay (for images to load)
setTimeout(() => {
    if (files.length > 0 && !primaryImageInput.value) {
        const firstPreview = previewContainer.querySelector('.variant-image-preview-item');
        if (firstPreview) {
            setVariantPrimaryImage(vIndex, firstPreview.dataset.filename, firstPreview);
        }
    }
     if (typeof lucide !== 'undefined') lucide.createIcons(); 
}, 150); 
...
const primaryImageInput = document.getElementById(`variants_${vIndex}_primary_image_filename`);
const previewContainer = document.getElementById(`variant_${vIndex}_image_preview_container`);
if (!input || !primaryImageInput || !previewContainer) return;

const dt = new DataTransfer();
const files = Array.from(input.files);
files.forEach(file => { if (file.name !== fileNameToRemove) dt.items.add(file); });
...
if (primaryImageInput.value === fileNameToRemove) { 
    ...
    if (remainingPreviews.length > 0) {
        ...
        setVariantPrimaryImage(vIndex, newPrimaryPreview.dataset.filename, newPrimaryPreview);
    }
}
if (typeof lucide !== 'undefined') lucide.createIcons();
}
function setVariantPrimaryImage(vIndex, fileName, clickedPreviewElement) {
const primaryImageInput = document.getElementById(`variants_${vIndex}_primary_image_filename`);
const previewContainer = document.getElementById(`variant_${vIndex}_image_preview_container`);
...
if (clickedPreviewElement) { 
     clickedPreviewElement.classList.add('is-primary');
} else { // Fallback if element not passed (e.g. called programmatically)
    const targetPreview = previewContainer.querySelector(`.variant-image-preview-item[data-filename="${fileName}"]`);
    if (targetPreview) targetPreview.classList.add('is-primary');
}
if (typeof lucide !== 'undefined') lucide.createIcons(); 
}
// --- Slugify Function (Keep existing) ---
// if (nameInput && slugInput) { /* ... */ }
// function slugify(text) { /* ... */ }
// (Copied from previous response for completeness)
if (productNameInput && slugInput) { productNameInput.addEventListener('keyup', () => { if (slugInput.placeholder === 'Tự động tạo nếu để trống' || slugInput.dataset.auto === "true") { slugInput.value = slugify(productNameInput.value); } }); slugInput.addEventListener('input', () => { slugInput.dataset.auto = slugInput.value.trim() === "" ? "true" : "false"; }); }
function slugify(text) { if (!text) return ''; return text.toString().toLowerCase().trim().normalize('NFKD').replace(/[\u0300-\u036f]/g, '').replace(/đ/g, 'd').replace(/\s+/g, '-').replace(/[^\w-]+/g, '').replace(/--+/g, '-').replace(/^-+/, '').replace(/-+$/, ''); }
...
if (productTypeRadio && productTypeRadio.value === 'variable' && Array.isArray(oldVariantsData) && oldVariantsData.length > 0) {
    ...
    oldVariantsData.forEach((oldVariant, loopIndex) => { // Use loopIndex to avoid conflict
        const originalGlobalIndex = variantIndexGlobal;
        // variantIndexGlobal = loopIndex; // Temporarily set global for this iteration - This was causing issues if oldVariantsData had non-sequential indices from form validation
        if (addVariantButton) addVariantButton.click(); // Creates card with data-variant-index="variantIndexGlobal" (which is now loopIndex effectively for this iteration *if* it's the first variant)
        // The card is created with the *current* variantIndexGlobal, then variantIndexGlobal is incremented.
        // So the card index for this iteration should be variantIndexGlobal - 1 (after click())
        // Or more reliably, just use loopIndex for selecting the card and its inputs.
        // The variantIndexGlobal should really be incremented *after* the click() in the addVariantButton handler.

        const currentVariantCard = variantsContainer ? variantsContainer.querySelector(`.variant-card[data-variant-index="${loopIndex}"]`) : null; // Use loopIndex to find the correct card
        if (currentVariantCard) {
            const skuInput = currentVariantCard.querySelector(`input[name="variants[${loopIndex}][sku]"]`);
            if(skuInput) skuInput.value = oldVariant.sku || '';
            const priceInput = currentVariantCard.querySelector(`input[name="variants[${loopIndex}][price]"]`);
            if(priceInput) priceInput.value = oldVariant.price || '';
            const salePriceInput = currentVariantCard.querySelector(`input[name="variants[${loopIndex}][sale_price]"]`);
            if(salePriceInput) salePriceInput.value = oldVariant.sale_price || '';
            const stockInput = currentVariantCard.querySelector(`input[name="variants[${loopIndex}][stock_quantity]"]`);
            ...
            if (oldVariant.attributes && typeof oldVariant.attributes === 'object') {
                Object.entries(oldVariant.attributes).forEach(([attrId, attrValueId]) => {
                    const attrSelect = currentVariantCard.querySelector(`select[name="variants[${loopIndex}][attributes][${attrId}]"]`);
                    if (attrSelect) attrSelect.value = attrValueId;
                });
            }
            const defaultRadio = currentVariantCard.querySelector(`input[name="variant_is_default_radio_group"][value="${loopIndex}"]`);
            const defaultHidden = currentVariantCard.querySelector(`input[name="variants[${loopIndex}][is_default]"]`);
            ...
            // Primary image filename from old data (won't show image, but keeps data if user re-uploads same named file)
            const primaryImageHiddenInput = currentVariantCard.querySelector(`input[name="variants[${loopIndex}][primary_image_filename]"]`);
            if (primaryImageHiddenInput && oldVariant.primary_image_filename) {
                // It's not possible to repopulate file inputs, so this value is mainly for backend reference if no new files are uploaded for this variant.
                // The frontend logic will rely on new file uploads to set the primary image.
                // primaryImageHiddenInput.value = oldVariant.primary_image_filename; 
            }
        }
        // variantIndexGlobal = originalGlobalIndex; // Restore global index - not needed if addVariantButton increments correctly
    });
    // variantIndexGlobal should now be equal to oldVariantsData.length after all old variants are processed by addVariantButton.click()
    updateDefaultVariantRadioAndHiddenFields();
}
});
...
const typeRadio = document.querySelector('input[name="type"]:checked');
if (!typeRadio) { showMessageModal('Cảnh báo', 'Vui lòng chọn loại sản phẩm.', 'error'); event.preventDefault(); return; }
const type = typeRadio.value;
    ...
    if (type === 'variable') {
        ...
        document.querySelectorAll('.variant-default-radio').forEach(radio => { if (radio.checked) oneDefaultRadioSelected = true; });
        ...
        for(let i=0; i < variantCards.length; i++) {
            ...
            const imageFilesInput = vCard.querySelector(`input[id="variants_${vIndexFromCard}_image_files"]`);
            const primaryImageFilenameInput = vCard.querySelector(`input[id="variants_${vIndexFromCard}_primary_image_filename"]`);

            if (imageFilesInput && imageFilesInput.files.length > 0 && (!primaryImageFilenameInput || !primaryImageFilenameInput.value)) {
                event.preventDefault();
                showMessageModal("Lỗi Biến Thể #" + (parseInt(vIndexFromCard)+1), "Vui lòng chọn một ảnh chính cho biến thể này (hoặc xóa tất cả ảnh của nó nếu không cần).", "error");
                vCard.scrollIntoView({ behavior: 'smooth', block: 'center' });
                const imageSection = vCard.querySelector(`#variant_${vIndexFromCard}_image_preview_container`);
                if(imageSection) { 
                    imageSection.style.outline = '2px solid red';
                    setTimeout(() => { imageSection.style.outline = ''; }, 3500);
                }
                return; 
            }
        }
...
// --- MutationObserver for Lucide Icons ---
...
if(variantsContainer) observer.observe(variantsContainer, { childList: true, subtree: true });
...
if(coverPreviewContainer) observer.observe(coverPreviewContainer, { childList: true });
if(galleryPreviewContainer) observer.observe(galleryPreviewContainer, { childList: true });
