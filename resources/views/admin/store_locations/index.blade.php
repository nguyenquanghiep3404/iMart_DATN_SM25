        ...
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-200">
                <div class="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center w-full">
                    <h3 class="text-xl font-semibold text-gray-900">Danh sách địa điểm (<span x-text="filteredLocations.length"></span>)</h3>
                </div>
            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider w-1/12">Trạng thái</th>
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm">
                                        ...
                                        <label class="relative inline-flex items-center cursor-pointer ml-2">
                                            <input type="checkbox" :checked="location.is_active" @change="toggleActive(location.id, location.is_active)" class="sr-only peer">
                                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-indigo-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
                                            <span class="ml-3 text-sm font-medium text-gray-900 hidden sm:inline" x-text="location.is_active ? 'Bật' : 'Tắt'"></span>
                                        </label>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div class="inline-flex space-x-2">
                                            <button @click="editLocation(location)" type="button" class="text-indigo-600 hover:text-indigo-900" title="Chỉnh sửa">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <button @click="openDeleteModal(location.id)" type="button" class="text-red-600 hover:text-red-900" title="Xóa">
                    ...
                    <form @submit.prevent="saveLocation()">
                        ...
                        <div class="grid grid-cols-1 gap-y-4 sm:grid-cols-2 sm:gap-x-6">
                            ...
                            <div class="sm:col-span-2">
                                <label for="address" class="block text-sm font-medium text-gray-700">Địa chỉ chi tiết (số nhà, đường)*</label>
                                <input type="text" id="address" name="address" x-model="formData.address"
                                        class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-3 py-2 mt-1" placeholder="Ví dụ: 123 Lê Lợi" required>
                            </div>
                            <div class="sm:col-span-2 flex items-center">
                                <label for="is_active_display" class="relative inline-block w-11 h-6 cursor-pointer">
                                    <input type="checkbox" id="is_active_display" x-model="formData.is_active" class="opacity-0 w-0 h-0">
                                    ...
                                    <span class="absolute content-[''] h-4 w-4 left-1 bottom-1 bg-white rounded-full transition-all duration-300"
                                          :class="{'translate-x-5': formData.is_active}"></span>
                                </label>
                                <span class="ml-3 text-sm text-gray-600" x-text="formData.is_active ? 'Đang hoạt động' : 'Ngưng hoạt động'"></span>
                            </div>
                        </div>
                        ...
                        <div class="mt-5 sm:mt-6 sm:flex sm:flex-row-reverse">
                            ...
                            <button @click.prevent="closeModal()" type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:w-auto sm:text-sm">
                                Hủy
                            </button>
                        </div>
                    </form>
        ...
        <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="isDeleteModalOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button @click="confirmDelete()" type="button" class="inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">
                        Xác nhận Xóa
                    </button>
