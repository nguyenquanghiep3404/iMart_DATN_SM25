...
@section('title', 'Chỉnh sửa Sản phẩm Cũ & Mở Hộp')

@push('styles')
<style>
    /* CSS giống hệt trang create */
...
<div class="body-content px-4 sm:px-6 md:px-8 py-8">
    <div class="container mx-auto max-w-4xl">
        <header class="mb-8">
            <h1 class="text-3xl font-bold text-gray-800">Chỉnh sửa Sản phẩm Cũ</h1>
        ...
        <form action="{{ route('admin.trade-in-items.update', $item) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="card-custom">
                ...
                <div class="card-custom-body grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Sản phẩm gốc (mới) <span class="text-red-500">*</span></label>
                        <div class="border border-gray-200 rounded-lg max-h-72 overflow-y-auto">
                            <div class="space-y-1 p-2">
                                @forelse($products as $product)
                                <div x-data="{ open: {{ in_array(old('product_variant_id', $item->product_variant_id), $product->variants->pluck('id')->toArray()) ? 'true' : 'false' }} }">
                                    ...
                                    <div x-show="open" x-transition class="mt-2 pl-4 space-y-2 border-l-2 border-indigo-200">
                                        ...
                                        <label class="flex items-center p-2 rounded-md hover:bg-indigo-50 cursor-pointer">
                                            <input type="radio" name="product_variant_id" value="{{ $variant->id }}" @checked(old('product_variant_id', $item->product_variant_id) == $variant->id) class="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500">
                    ...
                    <div>
                        ...
                        <input type="text" id="sku" name="sku" class="form-input" placeholder="Để trống để tự tạo" value="{{ old('sku', $item->sku) }}">
                    </div>
                    <div>
                        ...
                        <input type="text" id="imei_or_serial" name="imei_or_serial" class="form-input @error('imei_or_serial') border-red-500 @enderror" value="{{ old('imei_or_serial', $item->imei_or_serial) }}">
                    ...
                    <div>
                        ...
                        <input type="number" id="selling_price" name="selling_price" class="form-input @error('selling_price') border-red-500 @enderror" value="{{ old('selling_price', $item->selling_price) }}">
                    ...
                    <div>
                        ...
                        <select id="type" name="type" class="form-select">
                            <option value="used" @selected(old('type', $item->type) == 'used')>Máy đã qua sử dụng</option>
                            <option value="open_box" @selected(old('type', $item->type) == 'open_box')>Hàng mở hộp</option>
                        </select>
                    </div>
                    <div>
                        <label for="status" class="block text-sm font-medium text-gray-700 mb-1">Trạng thái</label>
                        <select id="status" name="status" class="form-select">
                            <option value="pending_inspection" @selected(old('status', $item->status) == 'pending_inspection')>Chờ kiểm tra</option>
                            <option value="available" @selected(old('status', $item->status) == 'available')>Sẵn sàng bán</option>
                            <option value="sold" @selected(old('status', $item->status) == 'sold')>Đã bán</option>
                        </select>
                    </div>
                </div>
            </div>
                ...
                <div class="card-custom-body grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select id="condition_grade" name="condition_grade" class="form-select">
                            <option value="A" @selected(old('condition_grade', $item->condition_grade) == 'A')>Loại A (Như mới)</option>
                            <option value="B" @selected(old('condition_grade', $item->condition_grade) == 'B')>Loại B (Trầy xước nhẹ)</option>
                            <option value="C" @selected(old('condition_grade', $item->condition_grade) == 'C')>Loại C (Trầy xước nhiều)</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select id="store_location_id" name="store_location_id" class="form-select @error('store_location_id') border-red-500 @enderror">
                            ...
                            @foreach($storeLocations as $location)
                                <option value="{{ $location->id }}" @selected(old('store_location_id', $item->store_location_id) == $location->id)>{{ $location->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="condition_description" name="condition_description" rows="4" class="form-textarea @error('condition_description') border-red-500 @enderror" placeholder="Ví dụ: Máy đẹp như mới, pin 99%...">{{ old('condition_description', $item->condition_description) }}</textarea>
...
@push('scripts')
<script>
    // --- Dữ liệu từ PHP để khôi phục form ---
    const initialImagesData = @json($images_data ?? [], JSON_UNESCAPED_UNICODE);
    const initialPrimaryImageId = @json($primary_image_id ?? null, JSON_UNESCAPED_UNICODE);
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Khôi phục lại các ảnh đã chọn
        populateInitialImages();
    });

    // --- CÁC HÀM XỬ LÝ ẢNH (giống hệt trang create) ---
    ...
    function populateInitialImages() {
        const imagesToRepopulate = Object.values(initialImagesData);
        if (imagesToRepopulate.length > 0) {
            addImagesToPreview(imagesToRepopulate);
            if (initialPrimaryImageId) {
                setPrimaryImage(parseInt(initialPrimaryImageId));
            }
        }
    }
</script>
