...
<style>
    ...
    .btn-outline-secondary { background-color: transparent; color: #6b7280; border: 1px solid #d1d5db; }
    ...
    /* Modal styles */
    .category-modal { display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.5); }
    .category-modal.show { display: flex; align-items: center; justify-content: center; }
    .category-modal-content { background-color: white; border-radius: 0.75rem; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04); width: 90%; max-width: 400px; }
    .category-modal-body { padding: 1.5rem; text-align: center; }
    .category-modal-footer { padding: 1rem 1.5rem; background-color: #f9fafb; border-top: 1px solid #e5e7eb; border-radius: 0 0 0.75rem 0.75rem; display: flex; justify-content: flex-end; gap: 0.75rem; }
        ...
        <div class="card-custom mb-6">
            <div class="card-custom-header">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Thùng rác Danh mục</h1>
                        <nav aria-label="breadcrumb" class="mt-2">
                            <ol class="flex text-sm text-gray-500">
                                <li><a href="{{ route('admin.categories.index') }}" class="text-indigo-600 hover:text-indigo-800">Danh sách Danh mục</a></li>
                    ...
                    <div class="flex flex-col md:flex-row gap-4 items-end">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="input-group">
                                <input type="text" name="search" id="search_category" class="form-input" placeholder="Nhập tên danh mục..." value="{{ request('search') }}">
                ...
                <div class="border border-gray-200 rounded-lg shadow-sm bg-white">
                    @if($trashedCategories->isEmpty())
                        <div class="text-center py-16">
                            <i class="fas fa-trash fa-4x text-gray-300 mb-4"></i>
                            <h5 class="text-lg font-medium text-gray-500 mb-2">Thùng rác trống</h5>
                            @if(request()->filled('search'))
                                <p class="text-sm text-gray-400 mb-4">Không tìm thấy danh mục nào với từ khóa "{{ request('search') }}"</p>
                                <a href="{{ route('admin.categories.trash') }}" class="btn btn-secondary">Xóa tìm kiếm</a>
                            @else
                                <p class="text-sm text-gray-400">Chưa có danh mục nào bị xóa.</p>
                            @endif
                        </div>
                    @else
                        <table class="table-custom table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 20%;" class="hidden md:table-cell">Người xóa</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div>
                                            ...
                                            <div class="text-xs text-gray-500 mt-1">
                                                <span class="font-medium">Danh mục cha:</span> {{ $category->parent?->name ?? 'Không có' }}
                                            </div>
                                            <div class="text-xs text-gray-500 md:hidden mt-1">
                                                <span class="font-medium">Xóa:</span> {{ $category->deleted_at->format('d/m/Y H:i') }} - 
                                                <span class="font-medium">Bởi:</span> {{ $category->deletedBy->name ?? 'Không rõ' }}
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="text-sm text-gray-600 hidden md:table-cell">
                                        {{ $category->deletedBy->name ?? 'Không rõ' }}
                                    </td>
                                    <td class="text-center">
                                        <div class="flex justify-center items-center space-x-1" style="min-width: 120px;">
                                            <button type="button" class="btn btn-success btn-sm" title="Khôi phục" onclick="confirmRestore({{ $category->id }}, '{{ $category->name }}')">
                                                <i class="fas fa-undo-alt"></i>
                                            </button>
                                            
                                            <button type="button" class="btn btn-danger btn-sm" title="Xóa vĩnh viễn" onclick="confirmForceDelete({{ $category->id }}, '{{ $category->name }}')">
            ...
            <div class="card-custom-footer">
                <div class="flex justify-between items-center">
                    <div class="text-sm text-gray-500">
                        Hiển thị {{ $trashedCategories->firstItem() }} - {{ $trashedCategories->lastItem() }} trong tổng số {{ $trashedCategories->total() }} kết quả
                    </div>
                    <div>
                        {{ $trashedCategories->appends(request()->query())->links() }}
                    </div>
                </div>
            </div>
            @endif
...
<div id="restoreModal" class="category-modal">
    <div class="category-modal-content animated-modal">
        <div class="category-modal-body text-center">
            ...
            <p class="text-sm text-gray-500 mb-4">Bạn có chắc chắn muốn khôi phục danh mục <strong id="categoryNameToRestore"></strong>?</p>
            <p class="text-xs text-gray-400">Danh mục sẽ được đưa trở lại danh sách chính.</p>
        </div>
...
let categoryIdToRestore = null;
let categoryIdToForceDelete = null;

// RESTORE MODAL FUNCTIONS
function confirmRestore(categoryId, categoryName) {
    categoryIdToRestore = categoryId;
    document.getElementById('categoryNameToRestore').textContent = categoryName;
...
function confirmForceDelete(categoryId, categoryName) {
    categoryIdToForceDelete = categoryId;
    document.getElementById('categoryNameToForceDelete').textContent = categoryName;
