...
@push('styles')
    {{-- Copy style từ trang users.edit nếu cần --}}
@endpush
...
<div class="body-content px-4 md:px-8 py-8">
    <div class="container mx-auto max-w-4xl">
        <div class="mb-8">
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900">Chỉnh sửa Vai trò</h1>
        </div>
        ...
        <div class="card-form">
            <form method="POST" action="{{ route('admin.roles.update', $role) }}">
                ...
                <div class="p-6 space-y-6">
                    <div>
                        <label for="role_name" class="form-label">Tên Vai trò <span class="text-red-500">*</span></label>
                        <input type="text" id="role_name" name="name" class="form-input @error('name') is-invalid @enderror" value="{{ old('name', $role->name) }}" required>
                        @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div>
                        <label for="role_description" class="form-label">Mô tả</label>
                        <input type="text" id="role_description" name="description" class="form-input @error('description') is-invalid @enderror" value="{{ old('description', $role->description) }}">
                                            ...
                                            @foreach ($groupPermissions as $permission)
                                                <div class="flex items-center">
                                                    <input type="checkbox"
                                                        ...
                                                        {{-- Kiểm tra xem vai trò này đã có quyền này chưa để tích sẵn --}}
