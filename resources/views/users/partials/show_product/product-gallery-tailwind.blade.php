<div class="image-gallery">
    ...
    <div class="relative items-center mt-4"
        style="max-width: 1000px; min-width: 600px; margin: 0 auto; display: flex; align-items: center; height: 120px;">
        <button id="thumbs-prev-btn" class="absolute left-0 z-10 bg-white/70 p-1 rounded-full shadow-md">
            <svg class="w-5 h-5 text-gray-800" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
        </button>
        <div id="main-thumbnails" class="flex items-center gap-3 overflow-x-auto scrollbar-hide px-2 py-1"
            style="width: auto; max-width: 1000px; height: auto; margin: 0 auto;">
            <!-- ảnh sẽ render ở đây -->
        </div>
...
@php
    ...
    // Lấy ảnh từ sản phẩm đơn giản (cover image)
    if ($product->coverImage) {
        $initialImages[] = Storage::url($product->coverImage->path);
    }
    
    // Lấy ảnh từ gallery của sản phẩm
    foreach ($product->galleryImages as $galleryImage) {
        $initialImages[] = Storage::url($galleryImage->path);
    }
    
    // Lấy ảnh từ biến thể nếu có
    if ($defaultVariant && $defaultVariant->images->count()) {
        $variantImages = $defaultVariant->images->map(fn($img) => Storage::url($img->path))->toArray();
        $initialImages = array_merge($initialImages, $variantImages);
    }
