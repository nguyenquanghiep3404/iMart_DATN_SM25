      ...
      <div class="row align-items-center justify-content-between">
        <div class="col-sm-7 col-md-8 col-xxl-9 d-flex align-items-center mb-3 mb-sm-0">
          ...
          <div class="dropdown ms-auto ms-sm-0">
            <button type="button" class="btn btn-icon btn-ghost btn-secondary border-0" id="wishlist-selector"
        ...
        <div class="product-card animate-underline hover-effect-opacity bg-body rounded">
          <div class="position-relative">
            <div class="position-absolute top-0 end-0 z-1 pt-1 pe-1 mt-2 me-2">
              <div class="form-check fs-lg">
                <input type="checkbox" class="form-check-input select-card-check" name="wishlist_ids[]" value="{{ $product->product_variant_id }}" checked>
          ...
          <div class="w-100 min-w-0 px-1 pb-2 px-sm-3 pb-sm-3">
            <div class="d-flex align-items-center gap-2 mb-2">
              ...
              <span class="text-body-tertiary fs-xs">({{ $product->productVariant->reviews_count ?? 0 }})</span>
            </div>
            <h3 class="pb-1 mb-2">
              <a class="d-block fs-sm fw-medium text-truncate" href="{{ route('shop.product.show', $product->productVariant->id) }}">
                <span class="animate-target">{{ $product->productVariant->name }}</span>
              </a>
            </h3>
            <div class="d-flex align-items-center justify-content-between">
              <div class="h5 lh-1 mb-0">
                ${{ number_format($product->productVariant->price, 2) }}
                @if($product->productVariant->original_price && $product->productVariant->original_price > $product->productVariant->price)
