              ...
              <tr data-item-id="{{ $item->id }}">
                <td class="py-3 ps-0">
                  <div class="d-flex align-items-center">
                    <a class="flex-shrink-0" href="shop-product-general-electronics.html">
                      <img src="{{ $item->productVariant->image_url ?? 'default.jpg' }}" alt="{{ $item->productVariant->product->name }}" class="w-20 h-20 object-cover rounded">
                    </a>
                    <div class="w-100 min-w-0 ps-2 ps-xl-3">
                      ...
                      @foreach ($item->productVariant->attributeValues as $attrValue)
                        <ul class="list-unstyled gap-1 fs-xs mb-0">
                          <li><span class="text-body-secondary">{{ $attrValue->attribute->name ?? 'Thuộc tính' }}</span> <span class="text-dark-emphasis fw-medium">{{ $attrValue->value }}</span></li>
                          <li class="d-xl-none"><span class="text-body-secondary">Price:</span> <span class="text-dark-emphasis fw-medium">{{ number_format($item->price, 0, ',', '.') }}đ</span></li>
                        </ul>
                      @endforeach
                ...
                <td class="py-3 d-none d-md-table-cell">
                  <div class="count-input">
                    <button type="button" class="btn btn-icon btn-decrement" aria-label="Decrement quantity">
                      <i class="ci-minus"></i>
                    </button>
                    <input type="number" class="form-control quantity-input" value="{{ $item->quantity }}" readonly>
                    <button type="button" class="btn btn-icon btn-increment" aria-label="Increment quantity">
                      <i class="ci-plus"></i>
                    </button>
                  </div>
                </td>       
                <td class="h6 py-3 d-none d-xl-table-cell item-subtotal" data-price="{{ $item->price }}">
...
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('tr[data-item-id]').forEach(function (row) {
    ...
    function updateQuantity(newQuantity) {
      ...
      subtotalCell.textContent = (price * newQuantity).toLocaleString('vi-VN') + 'đ';

      // Gửi request AJAX cập nhật database
