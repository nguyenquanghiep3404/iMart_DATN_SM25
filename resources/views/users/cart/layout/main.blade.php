...
@foreach ($items as $item)
    ...
    @php
        ...
        $productLink = $isNewProduct
            ? route('users.products.show', ['slug' => $productSlug, 'variant_id' => $item->id])
                    ...
                    @if ($isNewProduct && !empty($item->variant_attributes))
                        <ul class="list-unstyled gap-1 fs-xs mb-0">
                            @foreach ($item->variant_attributes as $attrName => $attrValue)
                                <li>
                                    <span class="text-body-secondary">{{ $attrName }}:</span>
                                    <span class="text-dark-emphasis fw-medium">{{ $attrValue }}</span>
                                </li>
                            @endforeach
                        </ul>
                    @endif
