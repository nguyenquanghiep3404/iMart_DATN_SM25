...
<script>
  ...
  document.querySelectorAll('tr[data-item-id]').forEach(function (row) {
    ...
    function updateUI(newQuantity) {
      ...
      const totalQty = Array.from(document.querySelectorAll('.quantity-input')).reduce((sum, input) => {
    ...
    function updateQuantity(newQuantity, oldQuantity) {
      ...
      .then(async res => {
        if (!res.ok) {
          if (res.status === 422) {
            ...
            showSlideAlert('error', err.message || 'Số lượng vượt quá tồn kho.');
      ...
      .then(data => {
        ...
        if (data.success) {
          showSlideAlert('success', data.message || 'Cập nhật số lượng thành công!');
          // Cập nhật tổng tiền
    ...
    row.querySelector('.btn-close')?.addEventListener('click', function () {
      ...
      .then(data => {
        if (data.success) {
          ...
          showSlideAlert('error', data.message || 'Xoá sản phẩm thất bại.');
        }
      })
      .catch(error => {
  ...
  if (voucherForm) {
    voucherForm.addEventListener('submit', function (e) {
      ...
      .then(async res => {
        ...
        if (!res.ok) {
          showSlideAlert('error', data.message || 'Mã giảm giá không hợp lệ!');
          return;
        }

        if (data.success) {
          showSlideAlert('success', data.message || 'Áp dụng mã giảm giá thành công!');
          // cập nhật giao diện
        } else {
          showSlideAlert('error', data.message || 'Không thể áp dụng mã giảm giá.');
        }
      })
      .catch(error => {
        console.error(error);
