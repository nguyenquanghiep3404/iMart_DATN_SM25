            ...
            <div class="row pt-1 pt-sm-3 pt-lg-4 pb-2 pb-md-3 pb-lg-4 pb-xl-5">
                <div class="col-lg-8 col-xl-7 mb-5 mb-lg-0">
                    <div class="d-flex flex-column gap-4">
                        ...
                        <div class="bg-white rounded shadow-sm p-4">
                                    ...
                                    <div class="col-md-6">
                                        <div class="option-card rounded p-3 selected" id="delivery-method-delivery">
                                            <div class="form-check">
                                                ...
                                                <label class="form-check-label fw-medium w-100"
                                                    for="delivery_method_radio_delivery">Giao hàng tận nơi</label>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                            ...
                            <div id="delivery-address-section">
                                            ...
                                            <div class="d-flex gap-2 flex-wrap">
                                                <button type="button" id="open-address-modal-btn"
                                                    class="btn btn-outline-secondary btn-sm"><i
                                                        class="fas fa-exchange-alt me-1"></i>Chọn từ sổ địa chỉ</button>
                                                <button type="button" id="use-new-address-btn"
                                                    class="btn btn-outline-dark btn-sm"><i class="fas fa-plus me-1"></i>Thêm địa
                                                    chỉ mới</button>
                                            </div>
                                        </div>
                                    @endif
                                ...
                                <div id="new-address-form-wrapper" class="d-none">
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        ...
                                        @auth
                                            @if (Auth::user()->addresses->isNotEmpty())
                                                <button type="button" id="back-to-address-list-btn"
                            ...
                            <div id="pickup-location-section" class="d-none">
                                ...
                                <div id="pickup-store-selection">
                                    <h3 class="h6 mb-3" style="color: black; font-weight: bold;">Chọn cửa hàng để nhận
                                        hàng <span class="text-danger">*</span></h3>
                                        ...
                                        <div class="border rounded p-3 bg-light">
                                            <div class="d-flex align-items-start">
                                                ...
                                                <div class="flex-grow-1">
                                                    ...
                                                    <span id="selected-store-address"
                                                        class="d-block small text-muted"></span>
                                                    <span id="selected-store-phone"
                                                        class="d-block small text-muted"></span>
                                                </div>
                                    ...
                                    <div id="store_location_error" class="error-message"><i
                                            class="fas fa-exclamation-circle"></i><span>Vui lòng chọn cửa hàng</span></div>
                                </div>
                        ...
                        <div class="bg-white rounded shadow-sm p-4">
                            <h2 class="h5 mb-4">2. Sản phẩm và Vận chuyển (<span
                                    id="product-count">{{ count($items) }}</span>)</h2>
                            <div id="main-product-list" class="mb-4"></div>
                            <div id="shipping_method_error" class="error-message mb-3"><i
                                    class="fas fa-exclamation-circle"></i><span>Vui lòng chọn phương thức vận chuyển</span>
                            </div>
                                ...
                                <ul class="list-unstyled mb-0">
                                    <li class="d-flex justify-content-between mb-2">
                                        ...
                                        <span id="cart-subtotal"
                                            class="text-dark fw-medium">{{ number_format($subtotal, 0, ',', '.') }}₫</span>
                                    </li>
                                    <li class="d-flex justify-content-between mb-2">
                                        <span class="text-muted small">Giảm từ voucher:</span>
                                        <span id="cart-discount"
                                            class="text-danger fw-medium">{{ $discount > 0 ? '-' . number_format($discount, 0, ',', '.') . '₫' : '0₫' }}</span>
                                    </li>
                                    <li class="d-flex justify-content-between mb-2" id="points-discount-row"
                                        @if (($pointsDiscount ?? 0) == 0) style="display: none;" @endif>
                                        <span class="text-muted small">Giảm từ điểm:</span>
                                        <span id="points-discount-amount"
                                            class="text-danger fw-medium">-{{ number_format($pointsDiscount ?? 0, 0, ',', '.') }}₫</span>
                                    </li>
        ...
        $checkout_config = [
            ...
            'user' =>
                Auth::check() && Auth::user()
                    ...
                    : null,
            'addresses' =>
                isset($userAddresses) && $userAddresses->isNotEmpty()
                    ? $userAddresses
                        ->map(function ($addr) {
                            return [
                                'id' => $addr->id,
                                ...
                                'province_code' =>
                                    $addr->address_system === 'new'
                                        ? $addr->new_province_code
                                        : $addr->old_province_code,
            ...
            'urls' => [
                'process' =>
                    isset($is_buy_now) && $is_buy_now
                        ? route('buy-now.process')
                        : (Auth::check()
                            ? route('payments.process')
                            : route('payments.process.guest')),
    ...
    <script>
        ...
        const CheckoutPage = {
            ...
            init(config) {
                ...
                this.state.selectedAddressId = this.state.addresses?.find(a => a.default === true || a.default === 1)
                    ?.id ||
            ...
            cacheElements() {
                // Main Sections
                this.elements.deliveryMethodCards = document.querySelectorAll(
            ...
            bindEvents() {
                ...
                document.addEventListener('click', (e) => {
                    if (e.target && e.target.id === 'confirm-store-selection-btn') {
                        ...
                        document.querySelectorAll('.store-item').forEach(item => item.classList.remove(
                            'selected'));
            ...
            setupInputValidation() {
                const fieldsToValidate = {
                    'full_name': {
                        validator: this.helpers.validateName,
                        message: 'Vui lòng nhập họ và tên'
                    },
                    'phone_number': {
                        validator: this.helpers.validatePhone,
                        message: 'Số điện thoại không hợp lệ'
                    },
                    'email': {
                        validator: this.helpers.validateEmail,
            ...
            setupInitialUI() {
                ...
                if (isDelivery) {
                    ...
                    if (this.state.isLoggedIn && this.state.hasAddresses) {
                        ...
                        this.elements.formIntroText.textContent =
                ...
                if (radio.value === 'pickup') {
                    if (this.state.selectedStore && this.state.selectedStore.id) {
                        ...
                        if (storeDisplay) {
                            ...
                            document.getElementById('selected-store-address').textContent = this.state.selectedStore
                                .address;
                            document.getElementById('selected-store-phone').textContent = this.state.selectedStore
                                .phone || '';
                        }
            ...
            handleAddressModalOpen() {
                ...
                `;
                    card.addEventListener('click', () => {
                        ...
                        listContainer.querySelectorAll('.address-card').forEach(c => c.classList.remove(
                            'selected'));
                        card.classList.add('selected');
                    });
            ...
            async renderShipments() {
                ...
                if (!destinationProvinceCode) {
                    this.elements.shipmentListSection.innerHTML =
                        '<p class="text-muted">Vui lòng chọn địa chỉ giao hàng để xem các gói hàng.</p>';
                    return;
                }
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        this.elements.shipmentListSection.innerHTML =
                            '<p class="text-danger">Có lỗi xảy ra khi tính toán gói hàng.</p>';
                    }
                } catch (error) {
            ...
            renderShipmentsFromAPI(shipments) {
                ...
                shipments.forEach(shipment => {
                    ...
                    const shippingOptionsContainer = shipmentElement.querySelector(
                        '.shipping-options-container');

                    shipmentElement.dataset.warehouseId = shipment.store_location_id;
                    shipmentElement.querySelector('h3').textContent =
                        `Gói hàng ${shipmentIndex}: Giao từ ${shipment.store_name || 'Cửa hàng'}`;
                    ...
                    shipment.items.forEach(product => {
                        ...
                        productNode.querySelector('.product-price').textContent = this.helpers
                            .formatCurrency(product.price);
                        productNode.querySelector('.product-quantity').textContent =
                            `x${product.quantity}`;
                        productContainer.appendChild(productNode);
                    });

                    this.renderShippingOptions(shippingOptionsContainer, shipment.store_location_id, shipment
                        .items);
            ...
            async getShippingOptionsForSelectedAddress() {
                ...
                if (!selectedAddress) {
                    this.elements.shipmentListSection.innerHTML =
                        '<p class="text-muted">Địa chỉ không hợp lệ. Vui lòng chọn lại địa chỉ giao hàng.</p>';
                    return;
                }
            ...
            async renderPickupOrderSummary() {
                ...
                if (!this.state.selectedStore || !this.state.selectedStore.id) {
                    if (this.elements.pickupSummarySection) {
                        this.elements.pickupSummarySection.innerHTML =
                            '<p class="text-muted">Vui lòng chọn cửa hàng nhận hàng để xem thông tin sản phẩm.</p>';
                    }
                    return;
                }
                ...
                this.elements.pickupSummarySection.innerHTML =
                    '<p class="text-muted"><i class="fas fa-spinner fa-spin me-2"></i>Đang tải thông tin gói hàng...</p>';
                ...
                try {
                    ...
                    if (result.success && result.shipments && result.shipments.length > 0) {
                        ...
                        if (result.unavailable_items && result.unavailable_items.length > 0) {
                            errorMessage +=
                                '<br><small>Một số sản phẩm không có đủ hàng tại cửa hàng được chọn.</small>';
                        }
            ...
            renderPickupShipmentsFromAPI(shipments) {
                ...
                if (!shipments || shipments.length === 0) {
                    ...
                    if (this.elements.pickupSummarySection) {
                        this.elements.pickupSummarySection.innerHTML =
                            '<p class="text-danger">Không có thông tin gói hàng.</p>';
                    }
                    return;
                }
                ...
                if (this.elements.pickupSummarySection && this.elements.pickupSummarySection.innerHTML.includes(
                        'Đang tải')) {
                ...
                shipments.forEach((shipment, index) => {
                    ...
                    if (shipment.requires_transfer) {
                        statusText =
                            `Sẽ chuyển từ ${shipment.source_name || 'kho hàng'} về ${shipment.pickup_store_name || 'cửa hàng đã chọn'}`;
                    ...
                    if (!shipment.items || !Array.isArray(shipment.items) || shipment.items.length === 0) {
                        ...
                        shipment.items.forEach(item => {
                            ...
                            if (!item.name || !item.image || !item.price) {
                                console.log('Tìm thông tin sản phẩm từ cartItems:', item
                                    .product_variant_id);
                                if (this.state && this.state.cartItems && Array.isArray(this.state
                                        .cartItems)) {
                                    ...
                                    if (cartItem) {
                                        ...
                                        console.warn(
                                            'Không tìm thấy thông tin sản phẩm trong cartItems');
                                    }
                                } else {
                                    console.error(
                                        'state.cartItems không tồn tại hoặc không phải là mảng');
                                }
                            }
            ...
            updateOrderInformation(options = {}) {
                ...
                this.elements.shippingFeeSummary.textContent = canCalculate ? this.helpers.formatCurrency(
                    totalShippingFee) : 'Chưa xác định';

                const finalTotal = this.state.subtotal - this.state.discount - this.state.pointsDiscount +
                    totalShippingFee;
                this.elements.grandTotalSummary.textContent = this.helpers.formatCurrency(finalTotal > 0 ? finalTotal :
                    0);
            },

            processOrder() {
                this.elements.placeOrderBtn.disabled = true;
                this.elements.placeOrderBtn.innerHTML =
                ...
                if (orderData.delivery_method === 'delivery') {
                    ...
                    document.querySelectorAll('.shipment-block').forEach(block => {
                        ...
                        if (deliveryTimeSlotSelect) shipmentData.delivery_time_slot = deliveryTimeSlotSelect
                            .value;
                        orderData.shipments.push(shipmentData);
                    });

                    if (!allValidShipments) {
                        this.helpers.showError('shipping_method',
                            'Vui lòng chọn phương thức vận chuyển hợp lệ cho tất cả gói hàng.');
                        this.helpers.resetOrderButton();
                        return;
                    }
            ...
            validateBeforeSubmit(orderData) {
                ...
                if (orderData.delivery_method === 'delivery') {
                    if (!this.state.selectedAddressId) {
                        ...
                        if (!name.value) {
                            this.helpers.showError('full_name', 'Vui lòng nhập họ và tên');
                            isValid = false;
                        }
                        const phone = this.elements.addressForm.querySelector('#phone_number');
                        if (!phone.value) {
                            this.helpers.showError('phone_number', 'Vui lòng nhập số điện thoại');
                            isValid = false;
                        }
                        const email = this.elements.addressForm.querySelector('#email');
                        if (!email.value) {
                            this.helpers.showError('email', 'Vui lòng nhập email');
                            isValid = false;
                        }
                        const province = this.elements.addressForm.querySelector('#province_id');
                        if (!province.value) {
                            this.helpers.showError('province_id', 'Vui lòng chọn Tỉnh/Thành phố');
                            isValid = false;
                        }
                        const district = this.elements.addressForm.querySelector('#district_id');
                        if (!district.value) {
                            this.helpers.showError('district_id', 'Vui lòng chọn Quận/Huyện');
                            isValid = false;
                        }
                        const ward = this.elements.addressForm.querySelector('#ward_id');
                        if (!ward.value) {
                            this.helpers.showError('ward_id', 'Vui lòng chọn Phường/Xã');
                            isValid = false;
                        }
                        const addressLine = this.elements.addressForm.querySelector('#address_line1');
                        if (!addressLine.value) {
                            this.helpers.showError('address_line1', 'Vui lòng nhập số nhà, tên đường');
                            isValid = false;
                        }
                    }
                    ...
                    if (!allShipmentsSelected) {
                        this.helpers.showError('shipping_method',
                            'Vui lòng chọn phương thức vận chuyển cho tất cả gói hàng.');
                        isValid = false;
                    }

                } else {
                    if (!this.elements.pickupFullName.value) {
                        this.helpers.showError('pickup_full_name', 'Vui lòng nhập họ và tên');
                        isValid = false;
                    }
                    if (!this.elements.pickupPhoneNumber.value) {
                        this.helpers.showError('pickup_phone_number', 'Vui lòng nhập số điện thoại');
                        isValid = false;
                    }
                    if (!this.elements.pickupEmail.value) {
                        this.helpers.showError('pickup_email', 'Vui lòng nhập email');
                        isValid = false;
                    }
                    if (!this.state.selectedStore) {
                        this.helpers.showError('store_location_error', 'Vui lòng chọn cửa hàng');
                        isValid = false;
                    }
            ...
            helpers: {
                ...
                loadProvinces: async () => {
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            data.data.forEach(p => {
                                select.innerHTML +=
                                    `<option value="${p.code}" data-name="${p.name}">${p.name_with_type}</option>`;
                            });
                ...
                loadDistricts: async (provinceCode) => {
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            data.data.forEach(d => {
                                select.innerHTML +=
                                    `<option value="${d.code}" data-name="${d.name}">${d.name_with_type}</option>`;
                            });
                            select.disabled = false;
                        } else {
                ...
                loadWards: async (districtCode) => {
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            data.data.forEach(w => {
                                select.innerHTML +=
                                    `<option value="${w.code}" data-name="${w.name}">${w.name_with_type}</option>`;
                            });
                            select.disabled = false;
                        } else {
                            select.innerHTML = '<option value="">Không có dữ liệu</option>';
                ...
                fetchGhnFee: async (provinceName, districtName, wardName, totalWeight) => {
                    ...
                    try {
                        const res = await fetch(window.checkoutConfig.urls.ghnFee, {
                            ...
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': window.checkoutConfig.csrfToken,
                                'Accept': 'application/json'
                            },
                            body: JSON.stringify({
                ...
                prefillUserInfo: (type) => {
                    ...
                    if (type === 'delivery') {
                        if (document.getElementById('full_name')) document.getElementById('full_name').value = user
                            .name || '';
                        if (document.getElementById('phone_number')) document.getElementById('phone_number').value =
                            user.phone || '';
                        if (document.getElementById('email')) document.getElementById('email').value = user.email ||
                            '';
                    } else if (type === 'pickup') {
                        if (document.getElementById('pickup_full_name')) document.getElementById('pickup_full_name')
                            .value = user.name || '';
                        if (document.getElementById('pickup_phone_number')) document.getElementById(
                            'pickup_phone_number').value = user.phone || '';
                        if (document.getElementById('pickup_email')) document.getElementById('pickup_email').value =
                            user.email || '';
                    }
                },
                ...
                handleBackendValidationErrors(errors) {
                    ...
                    Object.keys(errors).forEach(fieldName => {
                        ...
                        if (fieldName === 'shipping_method') {
                            ...
                            if (!firstErrorField) firstErrorField = document.getElementById(
            ...
            loadModalStoreLocations: async function(provinceCode = '', districtCode = '') {
                ...
                try {
                    ...
                    if (data.success && data.data.length > 0) {
                                ...
                                radioButtons.forEach(radio => {
                                    if (radio) {
                                        radio.addEventListener('change', () => {
                                            ...
                                            if (selectedItem) {
                                                selectedItem.classList.add('border-danger',
                                                    'bg-light');
                                            }
                                        });
                                    }
                        ...
                        modalStoreList.innerHTML =
                            '<div class="text-center py-4"><div class="text-warning mb-2"><i class="fas fa-store-slash"></i></div><p class="text-muted small mb-2">Không có cửa hàng nào trong khu vực này</p><p class="text-muted small">Vui lòng thử chọn tỉnh/thành phố khác</p></div>';
                    }
                } catch (error) {
                    ...
                    modalStoreList.innerHTML =
                        '<div class="text-center py-4"><div class="text-danger mb-2"><i class="fas fa-exclamation-triangle"></i></div><p class="text-danger small mb-2">Không thể tải danh sách cửa hàng</p><p class="text-muted small">Vui lòng thử lại sau hoặc liên hệ hỗ trợ</p></div>';
                }
            },
